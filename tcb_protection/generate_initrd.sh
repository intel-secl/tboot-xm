...
TA_HEX2BIN_BIN="${TA_HEX2BIN_BIN:-/opt/trustagent/share/hex2bin/bin}"
export WORKING_DIR
export TBOOTXM_BIN
export TBOOTXM_LIB
...
change_permissions() {
	...
	chmod 755 $TA_HEX2BIN_BIN/hex2bin
}
...
function check_prerequisites()
{
	...
	#Check for TA hex2bin
    if [ ! -e "$TA_HEX2BIN_BIN/hex2bin" ]; then
        echo "TA hex2bin File Not Found"
		if  [ $os_flavour == "ubuntu" ]
        then
            restore_config
        fi	
...
function revert_mkinitrd()
{
	...
	rm -rf /bin/measure /bin/tpmextend /lib/libwml.so /bin/hex2bin
...
function prepare_mkinitrd()
{
	...
	cp  $TA_HEX2BIN_BIN/hex2bin /bin/.
	chmod +x /bin/measure /bin/tpmextend /lib/libwml.so /bin/hex2bin
